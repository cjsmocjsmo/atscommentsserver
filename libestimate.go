...
func NewEstReqHandler(c echo.Context) error {
	...
	newdest := "/home/porthose_cjsmo_cjsmo/atscommentsserver/data/estimates/estimate_" + uuid + ".json"
	var est EstimateS
	est.UUID = uuid
...
func GlobEstimate() []EstimateS {
	accept, err := filepath.Glob("/home/porthose_cjsmo_cjsmo/atscommentsserver/data/estimates/*.json")
...
func CompleteEstReqHandler(c echo.Context) error {
	...
	Globestimates, err := filepath.Glob("/home/porthose_cjsmo_cjsmo/atscommentsserver/data/estimates/*.json")
	CheckError(err, "completed glob has failed")
	var AEst EstimateS
	for _, f := range Globestimates {
		if strings.Contains(query, f) {
			esti := ReadEstFile(f)
			newdest := "/home/porthose_cjsmo_cjsmo/atscommentsserver/data/estcompleted/estcompleted_" + esti.UUID + ".json"
